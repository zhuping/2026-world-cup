...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  return buildMetadata({ locale: params.lang, path: '/venues', pageKey: 'venues' });
}

export default function VenuesPage({ params }: { params: { lang: string } }) {
  const dict = getDictionary(params.lang);
  const copy = dict.venues;
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: copy.h1, item: buildUrl(lang, '/venues') },
    ],
  };
